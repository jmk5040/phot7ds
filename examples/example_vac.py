...
3. running EAzY for photometric redshifts,
4. running FAST++ for SED fitting,
5. merging everything into ``{tile}_{ref}_value_added.fits``.

This needs the optional ``vac`` extra plus the compiled EAzY and FAST++
binaries::

    pip install -e ".[vac]"
    python examples/example_vac.py

Edit the constants below for your system: the EAzY/FAST++ ``LIB`` tree,
the external reference catalogs and the photometric catalog itself must
all be supplied by the user.
"""
#%%
...
CATALOG_PATH = f"/path_cat/.../{TILE}/{TILE}_{DETECTION_REF}_phot.fits"

# Tile-definition table (polygon corners + center).
TILE_TABLE = "/path_cfg/.../7DT_tiles.fits"

# EAzY/FAST++ config tree, external reference catalogs and output root.
LIB_DIR = "/path_cfg/.../LIB"
CATALOG_DIR = "/path_cat/.../catalog"
OUTPUT_ROOT = "/path_res/.../vac"

# Compiled binaries. Leave as None to pick them up from $PATH (or from
# $PHOT7DS_FASTPP_BIN / $PHOT7DS_EAZY_BIN); set them explicitly when the
# executables live outside $PATH.
FASTPP_BIN = None
EAZY_BIN = None
